...
public class DepositBuilder implements IDepositBuilder {
	...
	public IDeposit buildRenewalNoCapitalizationDeposit(IInnerProduct product, IClient selectedClient,
		...
		IInnerDeposit innerDeposit = innerDepositBuilder.build(currency, interestCalculator, money,
				currentDate.createClone(), period);
		IDeposit renewalNoCapitalizationDeposit = new RenewalNoCapitalizationDeposit(product, selectedClient,
				innerDeposit);

		return new RenewableDeposit(renewalNoCapitalizationDeposit, this);
	}
	...
	public IDeposit buildRenewalCapitalizationDeposit(IInnerProduct product, IClient selectedClient, ICurrency currency,
		...
		IInnerDeposit innerDeposit = innerDepositBuilder.build(currency, interestCalculator, money,
				currentDate.createClone(), period);
		IDeposit renewalCapitalizationDeposit = new RenewalCapitalizationDeposit(product, selectedClient, innerDeposit);

		return new RenewableDeposit(renewalCapitalizationDeposit, this);
	}

	@Override
	public IDeposit buildNoRenewableDepositFromRenewableDeposit(IDeposit deposit) {
		return new NoRenewalDeposit(deposit);
	}
}
